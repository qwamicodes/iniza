...
Restorable and Synchronized are separate conclusions. Until a selected Project representation and Restore Rehearsal exist, the audit always reports a Restorable gap. Skipped checks, missing upstreams, ahead or diverged state, a missing advertised upstream branch, and unprotected local-only state appear as Synchronized gaps. A behind count remains visible but is informational when the live remote is authoritative. Remote absence or failure retains the local audit and selects full Project Capsule protection rather than discarding tracked state.

Protection classification is deterministic and Plan-bound. Local-only state or an unusable remote selects a full Project Capsule. A remotely reconstructed representation is eligible only when the Project is verified and unchanged, is clean, has no local-only references or stash, has zero locally known ahead commits, and the live remote advertises the configured upstream branch. The live advertised commit is authoritative even when the checkout is behind and becomes the exact reconstruction commit in the Plan. Every pending ignored-state candidate receives one explicit decision. The resulting revised Plan is unapproved and records the selected local overlay plus the exact remote reconstruction recipe; it cannot itself establish Restorable status.
