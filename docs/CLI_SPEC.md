...
The in-progress COKS-30 core prototype adds cooperative Pack pause and authenticated Resume. It is not yet an implemented `iniza pack --resume` command. Pause occurs after a complete Migration Item; Resume currently requires both Recovery Methods and re-encrypts saved progress into a fresh encryption domain. Crash reconciliation and complete persistence-failure coverage remain unfinished. See [the current design and open gates](adr/0008-prototype-authenticated-pack-resume.md) and [the synthetic testing guide](manual-qa/COKS-30-pack-resume-progress.md).
