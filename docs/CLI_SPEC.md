...
The completed COKS-30 core transaction adds cooperative Pack pause, repeated-interrupt escalation, authenticated Resume, atomic no-overwrite publication, persistence-failure containment, and deterministic recovery guidance. It is not yet an implemented `iniza pack --resume` command. Pause occurs after a complete Migration Item; Resume requires both Recovery Methods, authenticates all saved progress and current source observations, and re-encrypts saved progress into a fresh encryption domain. See [the accepted core design and remaining Owner Dogfood gates](adr/0008-prototype-authenticated-pack-resume.md) and [the synthetic testing guide](manual-qa/COKS-30-pack-resume-progress.md).
